...
static unsigned char jffs2_eof_mark[4] = { 
	0xde, 0xad, 0xc0, 0xde 
};
...
static char md5salt_normal[MD5SUM_LEN] = {
	0xdc, 0xd7, 0x3a, 0xa5, 0xc3, 0x95, 0x98, 0xfb,
	0xdd, 0xf9, 0xe7, 0xf4, 0x0e, 0xae, 0x47, 0x38,
};

static char md5salt_boot[MD5SUM_LEN] = {
...
static int 
get_md5(char *data, int size, char *md5)
{
	MD5_CTX ctx;
...
static int 
read_to_buf(struct file_info *fdata, char *buf)
{
	FILE *f;
...
check_options(void)
{
	...
	if (combined) {
		...
		if (rootfs_align) {
			...
			DBG("Container length aligned to %u", 
				kernel_len + rootfs_info.file_size);
				
			DBG("Constraint aligned to %u", 
				fw_max_len - sizeof(struct fw_header));	
...
static int 
pad_jffs2(char *buf, int currlen)
{
	int len;
	uint32_t pad_mask;
...
static inline void 
inspect_fw_pstr(char *label, char *str)
{
	printf("%-23s: %s\n", label, str);
}

static inline void 
inspect_fw_phex(char *label, uint32_t val)
{
	printf("%-23s: 0x%08x\n", label, val);
}

static inline void 
inspect_fw_phexpost(char *label,
...
static inline void 
inspect_fw_phexdec(char *label, uint32_t val)
{
	printf("%-23s: 0x%08x / %8u bytes\n", label, val, val);
}

static inline void 
inspect_fw_phexdecdef(char *label,
                                         uint32_t val, uint32_t defval)
{
...
static inline void 
inspect_fw_pmd5sum(char *label, uint8_t *val, char *text)
{
	int i;
